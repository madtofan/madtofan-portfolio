      ...
      {technologies.map((tech, techIndex) => (
        ...
        >
          <h3 className="text-lg font-semibold mb-4 relative inline-block">
            {tech.category}
            <span className="absolute -bottom-1 left-0 w-full h-0.5 bg-primary/50 rounded"></span>
          </h3>
          <div className="flex flex-wrap gap-2">
            {tech.skills.map((skill, skillIndex) => (
              <span
                key={skill}
                className="inline-flex items-center rounded-md bg-primary/10 px-2 py-1 text-sm font-medium text-primary ring-1 ring-inset ring-primary/20 transition-all hover:bg-primary/20 hover:scale-105"
                style={{
